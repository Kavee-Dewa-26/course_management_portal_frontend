...
import { createRoleRequest, getRoleRequestsForApplicant } from "@/lib/mock/roleRequests";
...
export default function ApplyStudentPage() {
  ...
  const hasPending = useMemo(() => {
    ...
    return getRoleRequestsForApplicant(user.uid).some(
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    if (!user) return;
    setSubmitting(true);
    const created = createRoleRequest({
  ...
  return (
    <div className="page">
      ...
      {hasPending ? (
        ...
        >
          <Icon name="clock" size={20} style={{ color: "var(--color-warning)", flexShrink: 0, marginTop: 2 }} />
          <div style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "var(--color-primary-2)" }}>
            ...
            <Link href="/my-requests" style={{ color: "var(--color-warning)", fontWeight: 600 }}>
              My Requests
            </Link>{" "}
            to track its status, or wait for an admin decision.
          </div>
        </div>
      ) : (
        <>
    ...
    >
      {FEATURES.map((f) => (
        ...
        >
          ...
          <h3 style={{ margin: "0 0 4px", fontFamily: "var(--font-heading)", fontSize: 15, color: "var(--color-primary)" }}>
            {f.title}
          </h3>
          <p style={{ margin: 0, fontFamily: "var(--font-body)", fontSize: 13, color: "var(--color-body-green)", lineHeight: 1.5 }}>
            {f.body}
          </p>
        </div>
      ))}
